...
public class UploadActivity extends AppCompatActivity {
    ...
    private void submit() {
        ...
        MultipartBody.Part imagePart = MultipartBody.Part.createFormData("image", "cover.png", RequestBody.create(MediaType.parse("multipart/form-data"), coverImageData));

        Call<UploadResponse> resp = api.submitMessage(Constants.STUDENT_ID, "", fromPart, toPart, contentPart, imagePart, Constants.token);
        ...
        resp.enqueue(new Callback<UploadResponse>() {
            ...
            public void onFailure(final Call<UploadResponse> call, final Throwable t) {
                t.printStackTrace();
                Toast.makeText(UploadActivity.this, "发送失败" + t.toString(), Toast.LENGTH_SHORT).show();
            }
        });
