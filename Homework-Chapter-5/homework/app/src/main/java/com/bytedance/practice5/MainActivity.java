...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                List<Message> messageList = getMessagesFromRemote(studentId);
                if (messageList != null) {
                    new Handler(getMainLooper()).post(new Runnable() {  // update ui in main Thread
                        @Override
                        public void run() {
    ...
    private List<Message> getMessagesFromRemote(String studentId) {
        ...
        String urlStr = "https://api-sjtu-camp-2021.bytedance.com/homework/invoke/messages";
        List<Message> result = null;
        try {
            if (studentId != null) {
                Log.d(TAG, studentId);
                urlStr += String.format("?%s=%s", "student_id", Constants.STUDENT_ID);
            }
