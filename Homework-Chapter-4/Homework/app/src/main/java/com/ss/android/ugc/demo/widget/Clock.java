...
//        Log.d("clock","signal");
    ...
    private void drawHoursValues(Canvas canvas) {
        ...
        int r = (int) (mCenterX * 0.8f);
        for(int i=1;i<=12;i++){
            float rad = (float) (Math.toRadians(FULL_ANGLE/12*i)-Math.PI/2);
            int x = (int) (mCenterX+r*Math.cos(rad));
            int y = (int) (mCenterX+r*Math.sin(rad) - ((paint.descent() + paint.ascent()) / 2));
...
//        Log.d("clock","drawNeedles "+canvas);
